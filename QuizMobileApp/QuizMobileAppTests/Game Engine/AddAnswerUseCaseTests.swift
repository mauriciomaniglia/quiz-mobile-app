...
class AddAnswerUseCaseTests: XCTestCase {
    ...
    func test_addAnswer_doNotSaveEmptyAnswer() {
        ...
        var newSavedAnswer = [String]()
        sut.addAnswer("") { newSavedAnswer = $0.savedAnswers }
    ...
    func test_addAnswer_deliversTheSavedAnswersInTheCorrectOrder() {
        ...
        var newSavedAnswer = [String]()
        sut.addAnswer("Answer1") { newSavedAnswer = $0.savedAnswers }
        sut.addAnswer("Answer2") { newSavedAnswer = $0.savedAnswers }
        sut.addAnswer("Answer3") { newSavedAnswer = $0.savedAnswers }
        sut.addAnswer("") { newSavedAnswer = $0.savedAnswers }
        sut.addAnswer("Answer4") { newSavedAnswer = $0.savedAnswers }
        
        XCTAssertEqual(newSavedAnswer, ["Answer1", "Answer2", "Answer3", "Answer4"])
    }
    
    func test_addAnswer_removesExtraSpacesFromNewAnswers() {
        let (sut, _) = makeSUT()
    ...
    func test_addAnswer_doNotRemovesSpacesBetweenTwoWordsFromNewAnswer() {
        let (sut, _) = makeSUT()
        
        var newSavedAnswer = [String]()
        sut.addAnswer("My Answer") { newSavedAnswer = $0.savedAnswers }
    ...
    func test_addAnswer_deliversTotalNumberOfCorrectAnswers() {
        let counter = CounterSpy(seconds: 1)
        let sut = QuizGameEngine(counter: counter, correctAnswers: ["Answer1", "Answer2"])
        
        var correctAnswerTotal = 0
        sut.addAnswer("Answer") { correctAnswerTotal = $0.correctAnswersTotal }
